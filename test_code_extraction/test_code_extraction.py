...
Functions:
    - get_code_block_regex(language: str = "python") -> re.Pattern:
...
def get_code_block_regex(language: str = "python") -> re.Pattern:
    """
    Returns a compiled regex pattern for extracting code blocks of the specified language.
...
def process_json_file(json_path: str, output_dir: str, categories: List[str], tests_per_category: int, language: str = "python", file_extension: str = ".py") -> int:
    ...
    if not all_code_blocks:
        ...
        try:
            save_code_blocks(all_code_blocks, output_dir, categories, tests_per_category, file_extension)
        except Exception as e:
